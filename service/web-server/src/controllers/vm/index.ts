...
async function execute(this: IVMController) {
  ...
  const bashFile: string = `${__dirname}/scripts/runExecute.sh`;
  const executeFile: string = `${appRoot.toString()}/resources/execute.sh`;
...
async function downloadLogs(this: IVMController) {
  const { vmName } = this.execution;
  const bashFile: string = `${__dirname}/scripts/downloadLogs.sh`;
...
async function executeBashOnVm(bashFile: string, options: any[] = []) {

<template>
  <el-card>
    <div class="queryCardDetails flexed" @click="toggleStepsTable()">
      <el-tooltip class="item" effect="dark" :content="query" placement="top">
        <span style="width: 300px;">{{ query | truncate(100) }}</span>
      </el-tooltip>

      <el-tooltip class="item" effect="dark" content="Min/Rep" placement="top">
        <span
          class="text-size-18"
        >{{ minSpan.duration | fixedMs }}/{{ minSpan.rep + 1 | ordinalise }}</span>
      </el-tooltip>

      <el-tooltip class="item" effect="dark" content="Median/Reps" placement="top">
        <span class="text-size-18">{{ median | fixedMs }}/{{ reps }}</span>
      </el-tooltip>

      <el-tooltip class="item" effect="dark" content="Max/Rep" placement="top">
        <span
          class="text-size-18"
    ...
    <section class="stepsTable" v-if="expanded">
      ...
      <step-line
        ...
        :stepSpans="filterStepSpans(stepNumber)"
        :padding=0
      />
    </section>
  </el-card>

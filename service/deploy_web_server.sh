...
gcloud compute ssh ubuntu@$INSTANCE_NAME --zone=$ZONE --command='cd ~/service/web-server && npm install && tmux new-session -d -s node_server "node ~/service/web-server/src/server.js 2>&1 | tee -a ~/logs/node_server.log "'